...
export function availableProviders() {
  return Object.keys(clients).length ? Object.keys(clients) : ['mock'];
}

export async function complete(provider, input, options = {}) {
  if (provider === 'mock') {
    return { text: `[MOCK MODE] Received: ${input}\nConnect a provider key in Vercel to get live model output.`, usage: { input_tokens: input.length, output_tokens: 18 } };
  }
