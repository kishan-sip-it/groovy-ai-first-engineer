...
const clients = {};
if (process.env.ANTHROPIC_API_KEY) clients.anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
if (process.env.OPENAI_API_KEY) clients.openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
...
export async function complete(provider, input, options = {}) {
  ...
  if (provider === 'gemini') {
    ...
    const response = await clients.gemini.models.generateContent({
      model: options.model || cfg.geminiModel,
      contents: input
    });
...
export async function* stream(provider, input, options = {}) {
  if (provider === 'anthropic') {
    if (!clients.anthropic) throw new Error('ANTHROPIC_API_KEY is not configured');
    const s = await clients.anthropic.messages.create({ model: options.model || cfg.anthropicModel, max_tokens: options.maxTokens || 1024, messages:[{role:'user',content:input}], stream:true });
    for await (const event of s) if (event.type === 'content_block_delta' && event.delta?.type === 'text_delta') yield event.delta.text;
    return;
  }
  if (provider === 'gemini') {
    if (!clients.gemini) throw new Error('GEMINI_API_KEY is not configured');
    const s = await clients.gemini.models.generateContentStream({ model: options.model || cfg.geminiModel, contents: input });
    for await (const chunk of s) if (chunk.text) yield chunk.text;
    return;
  }
  if (provider === 'openai') {
    if (!clients.openai) throw new Error('OPENAI_API_KEY is not configured');
    const s = await clients.openai.responses.create({ model: options.model || cfg.openaiModel, input, stream:true });
