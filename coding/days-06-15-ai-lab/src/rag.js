...
function localEmbedding(text, dims=128){const v=new Array(dims).fill(0);for(const token of text.toLowerCase().split(/\W+/).filter(Boolean)){let h=2166136261;for(const c of token){h^=c.charCodeAt(0);h=Math.imul(h,16777619)}v[Math.abs(h)%dims]+=1;}const n=Math.hypot(...v)||1;return v.map(x=>x/n);}
...
export async function ingest(file){
  const abs=path.resolve(file); const buf=await fs.readFile(abs); let text;
  if(file.toLowerCase().endsWith('.pdf')){const parsed=await pdf(buf);text=parsed.text;} else text=buf.toString('utf8');
...
export async function answerQuestion(question,file){
  ...
  const chunks=await ingest(file); const q=(await embed([question]))[0];
  const top=chunks.map(c=>({...c,score:cosine(c.embedding,q)})).sort((a,b)=>b.score-a.score).slice(0,3);
  const context=top.map(c=>`[Source chunk ${c.id}] ${c.text}`).join('\n\n');
  const provider=Object.keys({anthropic:process.env.ANTHROPIC_API_KEY,openai:process.env.OPENAI_API_KEY,gemini:process.env.GEMINI_API_KEY}).find(Boolean);
  ...
  const r=await complete(provider,prompt); return r.text;
}
